        ...
        const fetchProperty = async () => {
            ...
            if (data) {
                ...
                if (!data.isPublished) {
                    ...
                    setProperty(data);
                    // SEO: Dynamic page title for Google and browser tab
                    const price = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(data.price);
                    document.title = `${data.title} — ${price} | Lago Realty`;
                                    ...
                                    agents.map(agent => (
                                        <div key={agent.id} className="group p-4 bg-slate-50 hover:bg-brand-green/5 border border-slate-100 rounded-2xl transition-all">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <a href={`https://wa.me/584246926562`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-3 text-sm font-bold text-white bg-[#25D366] hover:bg-[#128C7E] px-6 py-3 rounded-2xl transition-all shadow-lg shadow-[#25D366]/30 group active:scale-95">
                                                 ...
                                                 <a 
                                                    href={agent.bookingUrl || `https://wa.me/584246926562?text=${encodeURIComponent(`Hola ${agent.name}, me gustaría programar una visita para ${property.title}`)}`}
                                                    target="_blank" 
                                                    rel="noreferrer" 
